...
func _init() -> void:
	...
	var scenarios := [
		{"name": "Doğru (yerçekimi+hava)", "gravity": true, "kick": false, "air": true, "drag": Physics.DRAG_K, "imp": 0.0},
		{"name": "Yalnız yerçekimi", "gravity": true, "kick": false, "air": false, "drag": 0.0, "imp": 0.0},
		{"name": "Yerçekimi + F (impetus)", "gravity": true, "kick": true, "air": false, "drag": 0.0, "imp": Physics.IMPETUS_ACC},
		{"name": "Yerçekimi+Hava+F", "gravity": true, "kick": true, "air": true, "drag": Physics.DRAG_K, "imp": Physics.IMPETUS_ACC},
	]
	for s in scenarios:
		var cur: Dictionary = Physics.simulate(s.gravity, s.kick, s.air, s.drag if s.air else Physics.DRAG_K, s.imp)
	...
	for rho_cd in [[1.2, 0.25], [1.225, 0.25], [1.2, 0.20], [1.2, 0.35], [1.2, 0.42]]:
		...
		print("rho=%.3f Cd=%.2f -> k_fiziksel = %.5f  (DRAG_K=%.3f)" % [rho, cd, k_phys, Physics.DRAG_K])
	# DRAG_K=0.025'in örtük Cd'si (m/r/rho sabit tutulup Cd geriye çözülür)
	var cd_implied: float = Physics.DRAG_K * m / (0.5 * 1.2 * A)
	print("Not: k_fiziksel = 0.5*rho*Cd*A/m ; A = pi*r^2 = %.5f m^2" % A)
	print("DRAG_K=%.3f -> örtük Cd (rho=1.2) = %.3f" % [Physics.DRAG_K, cd_implied])

	print("\n=== 3. YUVARLANMA SIRASINDA HAVA DİRENCİ + ROLL_FRICTION ÇAKIŞMASI ===")
	var with_air: Dictionary = Physics.simulate(true, false, true, Physics.DRAG_K, 0.0)
	# aynı senaryoyu air=false ile tekrar koş (yuvarlanırken sürtünme kaynağı yalnız ROLL_FRICTION)
	var without_air: Dictionary = Physics.simulate(true, false, false, Physics.DRAG_K, 0.0)
	...
	for pt in with_air["points"]:
		if absf(pt["p"].x - with_air["impact_x"]) < 0.01:
			vx_at_impact = pt["v"].x
			break
	var drag_acc_at_impact: float = Physics.DRAG_K * absf(vx_at_impact) * absf(vx_at_impact)
	print("Çarpma anı yatay hız ~ %.2f m/s -> o anki hava-direnci ivmesi ~ %.3f m/s^2 (ROLL_FRICTION=%.1f m/s^2, oran=%.1f%%)" %
		[vx_at_impact, drag_acc_at_impact, Physics.ROLL_FRICTION, 100.0 * drag_acc_at_impact / Physics.ROLL_FRICTION])

	print("\n=== 4. MENZİL TABLOSU + KALE UYUMU (GOAL_X=%.1f m, FieldView.gd) ===" % 34.0)
	...
	for s in scenarios:
		var res: Dictionary = Physics.simulate(s.gravity, s.kick, s.air, s.drag if s.air else Physics.DRAG_K, s.imp)
	...
	print("-> Doğru cevabın (real_path) iniş noktası = %.3f m; GOAL_X=34.0 m civarında olmalı (hit_bulls toleransı)." % real["impact_x"])

	print("\n=== 5. SEKME/YUVARLANMA GÖRSEL TUTARLILIK (doğru senaryo) ===")
	var r2: Dictionary = Physics.simulate(true, false, true, Physics.DRAG_K, 0.0)
	var bounces := 0
	var last_y := -1.0
	for pt in r2["points"]:
